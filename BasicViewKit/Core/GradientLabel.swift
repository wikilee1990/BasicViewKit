...
@IBDesignable open class GradientLabel: UILabel {
    ...
    private var _fromC: UIColor = UIColor(cgColor: PalletBoard.otc_gradient().first!) {
        didSet {
            setNeedsDisplay()
        }
    }
    private var _toC: UIColor = UIColor(cgColor: PalletBoard.otc_gradient().second!) {
        didSet {
            setNeedsDisplay()
        }
    }
...
extension GradientLabel: BasicViewKit.Drawing {
...
extension GradientLabel: Changing {
    ...
    public func new(direction: BasicViewKit.Direction) {
